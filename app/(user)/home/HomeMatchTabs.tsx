  ...
  if (matches.length === 0) {
    return (
      <div className="text-center py-8 bg-white rounded-2xl border border-dashed border-border">
        ...
        <Link href="/matches" className="text-primary text-sm font-bold mt-2 inline-block hover:underline">Browse Matches →</Link>
      </div>
    );
  }
      ...
      <div className="flex gap-2 bg-gray-100 rounded-xl p-1">
        {tabs.map((tab) => (
          <button
            ...
            className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab === tab ? "bg-white text-foreground shadow-sm" : "text-muted"}`}
          >
            {tab}
          </button>
        ))}
        ...
        return (
          <Link key={match.id} href={`/matches/${match.id}`}>
            <div className="card-base hover:border-primary/20 transition-all cursor-pointer flex items-center gap-3">
              {match.banner && <img src={match.banner} alt="" className="w-14 h-14 rounded-xl object-cover shrink-0" onError={(e) => (e.currentTarget.style.display = "none")} />}
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${statusColors[match.status] ?? "bg-gray-100 text-gray-500"}`}>{match.status}</span>
                  {match.isVipOnly && <span className="text-[10px] text-amber-600 font-bold">👑</span>}
                </div>
                ...
                <div className="flex items-center gap-3 text-xs text-muted mt-1">
                  ...
                  {match.publishRoom && match.roomId && <span className="text-green-600 font-bold">Room: {match.roomId}</span>}
                </div>
              </div>
